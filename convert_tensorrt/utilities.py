...
# def prepare_mask_and_masked_image(image, mask, device):
#     """
...
#     mask[mask < 0.5] = 0
#     mask[mask >= 0.5] = 1
#     mask = torch.from_numpy(mask).to(dtype=torch.float32).to(device).contiguous()
...
def prepare_mask_and_masked_image(image, mask, device):
    ...
    # image = image[None].transpose(0, 3, 1, 2)
    # image = torch.from_numpy(image).to(dtype=torch.float32).contiguous() / 127.5 - 1.0
    image = transform(image).unsqueeze(0).type(torch.float16).to(device)
    if isinstance(mask, Image.Image):
        mask = np.array(mask.convert("L"))
        mask = mask.astype(np.float16) / 255.0
        mask = mask[None]
    mask = mask[None] # torch.Size([1, 1, 1024, 768])
    mask[mask < 0.5] = 0
    mask[mask >= 0.5] = 1
    if isinstance(mask, np.ndarray):
        mask = torch.from_numpy(mask)
    mask = mask.to(dtype=torch.float16).to(device).contiguous()
...
def add_arguments(parser):
    ...
    parser.add_argument('--version', type=str, default="xl-1.0", choices=("1.4", "1.5", "dreamshaper-7", "2.0-base", "2.0", "2.1-base", "2.1", "xl-1.0", "xl-turbo", "svd-xt-1.1", "sd3", "cascade", "flux.1-dev"), help="Version of Stable Diffusion")
    ...
    parser.add_argument('--scheduler', type=str, default="DDPM", choices=("DDIM", "DDPM", "EulerA", "Euler", "LCM", "LMSD", "PNDM", "UniPC", "DDPMWuerstchen", "FlowMatchEuler"), help="Scheduler for diffusion process")
    parser.add_argument('--guidance-scale', type=float, default=2, help="Value of classifier-free guidance scale (must be greater than 1)")
    ...
    # ONNX export
    parser.add_argument('--onnx-opset', type=int, default=18, choices=range(7,20), help="Select ONNX opset version to target for exported models")
    parser.add_argument('--onnx-dir', default='onnx', help="Output directory for ONNX export")
    parser.add_argument('--reconvert-onnx', default='', help="Output directory for ONNX export")

    # Framework model ckpt
    parser.add_argument('--framework-model-dir', default='/workspace/Try-on-Product/projects/try-on-ai/ckpts/tryon', help="Directory for HF saved models")
    ...
    parser.add_argument('--torch-inference', default=False, help="Run inference with PyTorch (using specified compilation mode) instead of TensorRT.")
...
def process_pipeline_args(args):
    ...
    kwargs_load_engine = {
        ...
        'reconvert_onnx': args.reconvert_onnx
    }
    ...
    args_run_demo = (args.height, args.width, args.batch_size, args.batch_count, args.num_warmup_runs, args.use_cuda_graph)
